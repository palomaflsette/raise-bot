...
def _update_rgb_frame_fast(gui):
    ...
    #     mask_rgb = extract_green_mask(rgb_frame)
    ...
    try:
        ...
        mask = extract_green_mask(gui._rgb_raw)
        cv2.imshow("Debug da Mascara de Cor", mask)
        cv2.waitKey(1)

        aruco_dict = cv2.aruco.getPredefinedDictionary(cv2.aruco.DICT_4X4_50)
        corners, ids, _ = cv2.aruco.detectMarkers(rgb_frame, aruco_dict)
        if ids is not None:
            cv2.aruco.drawDetectedMarkers(rgb_frame, corners, ids)
            cv2.putText(rgb_frame, "ARUCO PRESENTE", (10, 40), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 255, 0), 2)

        # O resto da função continua igual: desenha os alvos e atualiza o canvas.
...
def draw_target_point(frame, point_tuple):
    """Desenha um único alvo na tela."""
    if not point_tuple:
        return
   
    u, v = point_tuple[3], point_tuple[4]
    
    cv2.drawMarker(frame, (int(u), int(v)), (0, 255, 0), 
                   markerType=cv2.MARKER_CROSS, markerSize=20, thickness=2)

def start_candidate_thread(gui):
    if not hasattr(gui, '_camera_running'): gui._camera_running = True
    gui.last_known_point = None
    gui.candidate_points = []
    ...
    def update_loop():
        ...
        while getattr(gui, '_camera_running', False):
            try:
                ...
                if any(getattr(gui, attr) is None for attr in ['last_depth_frame', 'T_cam_to_robo', 'pincher', '_rgb_raw']):
                    time.sleep(0.5); continue

                novos_pontos = get_candidate_points(
                ...
                if novos_pontos:
                    if not target_locked:
                        print("lvo estável TRAVADO! Botão de movimento habilitado.")
                        gui.after(0, lambda: gui.move_button.configure(state="normal", fg_color="#2E8B57"))
                        target_locked = True
                    gui.last_known_point = novos_pontos[0]
                
                if gui.last_known_point:
                    gui.candidate_points = [gui.last_known_point]
                else:
                    gui.candidate_points = []
            except Exception as e: 
                print(f"[ERRO CRÍTICO NA THREAD DO ALVO] Ocorreu uma exceção: {e}")
            time.sleep(0.1)
