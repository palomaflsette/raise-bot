...
def HX(phi=0):
    """Returns a 4x4 transformation matrix for a rotation about the X axis."""
    return H(RX(phi))
def HY(phi=0):
    """Returns a 4x4 transformation matrix for a rotation about the Y axis."""
    return H(RY(phi))
def HZ(phi=0):
    """Returns a 4x4 transformation matrix for a rotation about the Z axis."""
    return H(RZ(phi))
def HT(T=[0,0,0]):
    """Returns a 4x4 transformation matrix with translation T."""
    return H(T=T)
    
def xaxis(h):
    """Returns the x axis of a transformation matrix."""
    return h[0:3,0]
def yaxis(h):
    """Returns the y axis of a transformation matrix."""
    return h[0:3,1]
def zaxis(h):
    """Returns the z axis of a transformation matrix."""
    return h[0:3,2]
def origin(h):
